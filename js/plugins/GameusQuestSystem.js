...
// 追蹤並關注各種他們可能遭遇到的不同任務。本系統也同樣給予一些實用的腳本指令
...
// 
// 安裝指示
// 將程式碼放置在插件資料夾下並另存為'GameusQuestSystem.js'，
// 按照你的需求配置選項即可。
//
// 我非常建議你可以參看本插件的範例檔(附帶任務編輯器)：
//  http://jugglingcode.com/scripts/MV/QuestSystem/Demo.zip
//
// 致謝名單
// gameus ~ 本插件製作者
...
//---------------------------------------------------------------------------------------------
    ...
    Window_QuestInfo.prototype.createQuestBitmap = function() {
     ...
     Window_QuestInfo.prototype.drawQuestInfo = function(q) {
        ...
        this.questBitmap.textColor = GameusScripts["Config"]["QuestSystem"]["Quest Name Color"] || this.systemColor();
        ...
        this.write();
        this.questBitmap.textColor = GameusScripts["Config"]["QuestSystem"]["Quest Description Color"] || this.normalColor();
